...
namespace
{
    const char* iconVS =
        ...
        "out vec2 oe_MyPlaceNode_texcoord; \n"
        "void oe_MyPlaceNode_icon_VS(inout vec4 vertex) \n"
        "{ \n"
        "    oe_MyPlaceNode_texcoord = gl_MultiTexCoord0.st; \n"
        "} \n";

    const char* iconFS =
        "#version " GLSL_VERSION_STR "\n"
        "in vec2 oe_MyPlaceNode_texcoord; \n"
        "uniform sampler2D oe_MyPlaceNode_tex; \n"
        "void oe_MyPlaceNode_icon_FS(inout vec4 color) \n"
        "{ \n"
        "    color = texture(oe_MyPlaceNode_tex, oe_MyPlaceNode_texcoord); \n"
        "} \n";
}

osg::observer_ptr<osg::StateSet> MyPlaceNode::s_geodeStateSet;
osg::observer_ptr<osg::StateSet> MyPlaceNode::s_imageStateSet;

MyPlaceNode::MyPlaceNode() :
GeoPositionNode()
{
    construct();
    compile();
...
MyPlaceNode::MyPlaceNode(const std::string& text,
                     const Style& style,
                     osg::Image* image) :
GeoPositionNode()
{
...
MyPlaceNode::MyPlaceNode(const GeoPoint& position,
...
MyPlaceNode::construct()
{
    ...
    if (s_imageStateSet.lock(_imageStateSet) == false)
    {
        ...
        if (s_imageStateSet.lock(_imageStateSet) == false)
        {
            ...
            vp->setName("MyPlaceNode::imageStateSet");
            vp->setFunction("oe_MyPlaceNode_icon_VS", iconVS, ShaderComp::LOCATION_VERTEX_MODEL);
            vp->setFunction("oe_MyPlaceNode_icon_FS", iconFS, ShaderComp::LOCATION_FRAGMENT_COLORING);
            _imageStateSet->addUniform(new osg::Uniform("oe_MyPlaceNode_tex", 0));
        }
    }
}
...
void
MyPlaceNode::setPriority(float value)
{
...
void
MyPlaceNode::updateLayoutData()
{

	osgEarth::ScreenSpaceLayoutOptions opts = osgEarth::ScreenSpaceLayout::getOptions();
	opts.sortByDistance() = true;
...
void
MyPlaceNode::setStyle(const Style& style, const osgDB::Options* readOptions)
{
    // changing the style requires a complete rebuild.
    _style = style;
...
MyPlaceNode::MyPlaceNode(const Config&         conf,
...
void
MyPlaceNode::setConfig(const Config& conf)
{
...
#undef  LC
#define LC "[MyPlaceNode Serializer] "

#include <osgDB/ObjectWrapper>
#include <osgDB/InputStream>
#include <osgDB/OutputStream>

namespace osgEarth { namespace Serializers { namespace MyPlaceNode
{
    // functions
    static bool checkConfig(const osgEarth::Annotation::MyPlaceNode& node)
    {
        return true;
    }

    static bool readConfig(osgDB::InputStream& is, osgEarth::Annotation::MyPlaceNode& node)
    {
        std::string json;
        is >> json;
        Config conf;
    ...
    static bool writeConfig(osgDB::OutputStream& os, const osgEarth::Annotation::MyPlaceNode& node)
    {
    ...
    REGISTER_OBJECT_WRAPPER(
        MyPlaceNode,
        new osgEarth::Annotation::MyPlaceNode,
        osgEarth::Annotation::MyPlaceNode,
        "osg::Object osg::Node osg::Group osgEarth::Annotation::AnnotationNode osgEarth::Annotation::GeoPositionNode osgEarth::Annotation::MyPlaceNode")
    {
