...
OSGRenderer::OSGRenderer(QQuickItem *parent)
    : QSGRenderNode(), QObject(parent), osgViewer::Viewer(), mOSGItem(parent)
{
...
OSGRenderer::OSGRenderer(osg::ArgumentParser* arguments, QQuickItem* parent)
    : QSGRenderNode(), QObject(parent), osgViewer::Viewer(*arguments), mOSGItem(parent)
{
    //    QObject::connect(QCoreApplication::instance(), &QCoreApplication::aboutToQuit,
    //                     [this]()
    //    {
...
void OSGRenderer::render(const RenderState *state)
{
    if (mOSGItem){
...
void OSGRenderer::releaseResources()
{

}

QSGRenderNode::StateFlags OSGRenderer::changedStates() const
{
    return DepthState|BlendState | ScissorState | StencilState |ColorState|BlendState|CullState|ViewportState|RenderTargetState;
}

QSGRenderNode::RenderingFlags OSGRenderer::flags() const
{
    return BoundedRectRendering ;
}

QRectF OSGRenderer::rect() const
{
    return QRect(0, 0, mWidth, mHeight);
}

void OSGRenderer::sync(QQuickItem *item)
{
